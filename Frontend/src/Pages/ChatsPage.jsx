    ...
    const fetchFriends = async () => {
      try {
        const res = await fetch(
          `http://localhost:5000/api/connections/${loggedInUserEmail}`
        );
        ...
        if (res.ok) {
          ...
          const connectedFriends = data.connections
            ...
            .map((conn) =>
              conn.email_user1.email === loggedInUserEmail
                ? conn.email_user2
                : conn.email_user1
            );
        ...
        {friends.length > 0 ? (
          friends.map((friend, index) => (
            <div
              ...
              onClick={() => navigate(`/chat/${friend.email}`)} // Navigate to the chat window of the friend
            >
              <img
                src={
                  friend.profilePhotoUrl || "https://via.placeholder.com/150"
